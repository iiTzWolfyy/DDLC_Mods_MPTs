...
layeredimage monika forward: #All definitions are for her facing forward.
    ...
    group left:
        ...
        ###ADD
        attribute ldown default if_any(["swimsuit"]): 
            path + "base_forward/1l_swimsuit.png"
        ...
        ###ADD BOTH FOLLOWING ATTRIBUTES
        # left hand with her index pointing like "Okay Everyone!"
        attribute lpoint if_any(["swimsuit"]): 
            path + "base_forward/2l_swimsuit.png"
        # left hand with the "peace" sign
        attribute lpeace if_any(["swimsuit"]): 
            path + "base_forward/3l_swimsuit.png"
    ...
    group right:
        ...
        ###ADD THIS ONE
        attribute rdown default if_any(["swimsuit"]):
            path + "base_forward/1r_swimsuit.png"    
        ...
        ###ADD THIS ONE
        # right hand on her hip in her swimsuit outfit
        attribute rhip if_any(["swimsuit"]):
            path + "base_forward/2r_swimsuit.png"

    ###ADD ALL THIS GROUP 'head' AND REMOVE THE 'always "mod_assets/MPT/monika/monika_forward_facebase.png"' STATEMENT ABOVE  
    group head:
        anchor (0,0) subpixel (True)
        attribute head default if_any(["uniform", "casual"]):
            "mod_assets/MPT/monika/monika_forward_facebase.png"

        attribute head default if_any(["swimsuit"]):
            path + "base_forward/facebase.png" # If you use the 'swimsuit' attribute (show monika forward swimsuit...)
...
layeredimage monika lean:
    ...
    group outfit: #These attributes are here only to determine which set of "body" sprites to use later.  "null" is what lets us just use these attributes as logic and nothing else.
        ...
        attribute uniform default null
        attribute casual null
        attribute swimsuit null ###ADD - Both hands behind her back
        attribute swimsuit_cheek null ###ADD - Same but her hand is on her cheek
    ...
    group body:
        ...
        ###ADD BOTH ATTRIBUTES BELOW
        # without her hand on her cheek
        attribute body default if_any(["swimsuit"]):
            path + "base_leaning/1.png"
        # with her hand on her cheek
        attribute body default if_any(["swimsuit_cheek"]):
            path + "base_leaning/2.png"
...
###ADD ALL THIS LAYEREDIMAGE AS WELL
layeredimage monika turned swimsuit:

    at renpy.partial(Flatten, drawable_resolution=False)

    always path + "base_turned/facebase.png" # Always using the same face

    always path + "base_turned/1r_turned.png"# and always the same right body part lol

    # This group mood's only purpose is to be used in the 'eyes', 'mouth' and 'eyebrows' groups, but you'll never write it,
    # since Monika doesn't really have much expressions for that pose, she's basically always happy eh
    group mood:
    ...
    group left:
        ...
        attribute ldown default:
            path + "base_turned/1l_turned.png"
        attribute lpoint:
            path + "base_turned/2l_turned.png"
        attribute lhand:
            path + "base_turned/3l_turned.png"
    
    # Yes, this group here is pretty much useless, I could've used an 'always' statement, but in case we add more eyebrows later on...
    group eyebrows:
        attribute brow default if_any(["neut"]):
            path + "eyebrows_turned/eb1.png"

    group eyes:
        # Opened eyes (default)
        attribute oe default if_any(["neut"]):
            path + "eyes_turned/e1a.png"

        # Closed eyes
        attribute ce if_any(["neut"]):
            path + "eyes_turned/e2a.png"

    group mouth:
        # Close mouth (default)
        attribute cm default if_any(["neut"]):
            path + "mouth_turned/ma.png"

        # Opened mouth
        attribute om if_any(["neut"]):
            path + "mouth_turned/mb.png"